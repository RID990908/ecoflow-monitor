"""Dashboard web (HTTP puro, sin framework) + toda la lógica de cálculo de
métricas/mensajes que alimenta tanto ese dashboard como el bot de Telegram
(_gather_metrics, build_report, build_load_advisor_message, etc. se generan
acá una sola vez y las consume también telegram_bot.py). Extraído de
ecoflow_telegram_monitor.py al modularizar el proyecto.

get_dashboard_status() se separó en varias funciones más chicas al mover el
código acá: _dashboard_battery_view/_dashboard_goal_view/_dashboard_staleness_view
juntan los datos crudos (batería, meta, frescura del dato), y
get_dashboard_status() arma el dict final que se serializa a JSON — mismo
resultado que antes para las mismas entradas, solo con las responsabilidades
separadas en piezas más chicas.
"""
...
import mqtt_client
import shared_state
from shared_state import DEVICE_CHARGED, DEVICE_INFO, DEVICE_STATE, ECOFLOW_READY, MULTI_UNIT_DEVICES, SN_DELTA2, TZ, log
...
def _last_ac_line() -> str:
    if not shared_state.LAST_AC_TIMESTAMP:
        return "⚡ Última vez que llegó corriente: sin registro"
    return f"⚡ Última vez que llegó corriente: hace {_format_elapsed(time.time() - shared_state.LAST_AC_TIMESTAMP)}"
...
def _gather_metrics(passive: bool = False) -> dict:
    ...
    seguido)."""
    data = mqtt_client.get_device_quota_cached(SN_DELTA2) if passive else mqtt_client.get_device_quota(SN_DELTA2)
    ...
    bms_slave_last_seen = mqtt_client._bms_slave_last_seen.get(SN_DELTA2)
    ...
    if soc_extra is not None:
        threshold_line = _time_to_threshold_line(avg_soc, system_net_w, 2, shared_state.BATTERY_LOW_THRESHOLD)
        threshold_eta = _time_to_threshold_eta(avg_soc, system_net_w, 2, shared_state.BATTERY_LOW_THRESHOLD)
    else:
        threshold_line = _time_to_threshold_line(soc_delta2, delta2_net_w, 1, shared_state.BATTERY_LOW_THRESHOLD)
        threshold_eta = _time_to_threshold_eta(soc_delta2, delta2_net_w, 1, shared_state.BATTERY_LOW_THRESHOLD)
...
def _multi_unit_line(emoji: str, label: str, device_keys: list, available_w) -> tuple:
    ...
    su propio watiaje): un punto 🟢/🔴 por unidad (ver _multi_unit_fits). El
    marcado real (/on-/off) va pegado a cada dot con un ✓ — son datos
    distintos (uno es "cuánto aguanta el sistema", el otro es "qué tenés
    prendido de verdad") pero van juntos en vez de un conteo aparte. Si
...
def _compute_device_fits(m: dict = None, now=None) -> dict:
    ...
    porque esa función arma texto línea por línea y mezclar ambas cosas la
    hacía más difícil de leer/tocar sin romper el mensaje del bot. Si el
    ORDEN de prioridad cambia, hay que tocarlo en los dos lados (acá y en
    build_load_advisor_message) — los cálculos de watts en sí (_allocate_budget,
    _multi_unit_fits) sí son compartidos y no se duplican."""
    now = now or datetime.now(TZ)
    if m is None:
...
def _log_checkpoint_result(now, avg_soc) -> None:
    ...
    today = now.date()
    for cp_min, floor, label in BATTERY_CHECKPOINTS:
        if cp_min <= minute_of_day < cp_min + 10 and _checkpoint_logged_for.get(cp_min) != today:
            diff = avg_soc - floor
            if diff >= 0:
...
def _dashboard_battery_view(m: dict) -> dict:
    """Parte de get_dashboard_status que solo junta datos crudos de batería
    (remain de cada una, ETA de carga/descarga) a partir de las métricas ya
    calculadas por _gather_metrics — separado del armado del dict final para
    que cada pieza tenga una sola responsabilidad."""
...
def _dashboard_goal_view(m: dict, now) -> dict:
    """Parte de get_dashboard_status que junta la proyección de meta del
    próximo checkpoint (mismo cálculo que la alerta dinámica de batería,
    ver _project_to_checkpoint) para mostrarla en el aro del dashboard."""
...
def _dashboard_staleness_view() -> dict:
    """Parte de get_dashboard_status que junta la frescura real de la
    telemetría MQTT (ver mqtt_client._mqtt_staleness, mismo criterio que
    watchdog_timer), no solo "el fetch HTTP al propio servidor respondió".
    ...
    is_stale, stale_minutes = mqtt_client._mqtt_staleness()
    stale_minutes = round(stale_minutes, 1) if stale_minutes is not None else None
    return {"stale": is_stale, "stale_minutes": stale_minutes}
...
def get_dashboard_status() -> dict:
    ...
    now = datetime.now(TZ)
    battery_view = _dashboard_battery_view(m)
    goal_view = _dashboard_goal_view(m, now)
    staleness_view = _dashboard_staleness_view()

    return {
        "ready": True,
        "percent": battery_view["percent"],
        ...
        "delta2_remain": battery_view["delta2_remain"],
        "extra_remain": battery_view["extra_remain"],
        "has_ac": m["has_ac"],
        "in_w": m["total_in_w"],
        "out_w": m["out_w"],
        "eta_text": battery_view["eta_text"],
        "eta_ok": battery_view["eta_ok"],
        "remain_duration": battery_view["remain_duration"],
        ...
        "threshold_short": (
            f"{shared_state.BATTERY_LOW_THRESHOLD}% a las {m['threshold_eta'].strftime('%H:%M')}" if m["threshold_eta"] else None
        ),
        ...
        "stale": staleness_view["stale"],
        "stale_minutes": staleness_view["stale_minutes"],
        "goal_label": goal_view["goal_label"],
        "goal_floor": goal_view["goal_floor"],
        "goal_projected": goal_view["goal_projected"],
        "goal_met": goal_view["goal_met"],
    }
...
<body>
  ...
  <div class="flow-top-wrap">
    <div class="icons-row top">
      <div class="icon-item">
        <div class="icon-circle" id="ac-circle"><svg class="tower-svg" viewBox="0 0 24 24" width="24" height="24"><defs><linearGradient id="towerGradAc" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#c2d0d8"/><stop offset="1" stop-color="#7c8f9c"/></linearGradient><linearGradient id="wingGradAc" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4a6072"/><stop offset="1" stop-color="#1e2a35"/></linearGradient><linearGradient id="insulatorGradAc" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#9df0fb"/><stop offset="1" stop-color="#1fa8c2"/></linearGradient></defs><rect x="3.5" y="21" width="17" height="1.8" rx="0.9" fill="#12161b"/><rect x="3.5" y="21" width="17" height="0.6" rx="0.3" fill="#333b43" opacity="0.7"/><path d="M12,1.6 L7.2,21 M12,1.6 L16.8,21" stroke="url(#towerGradAc)" stroke-width="1.7" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.3,8.6 L14.7,8.6 M9.3,8.6 L14.7,14 M14.7,8.6 L9.3,14 M8.3,14 L15.7,14 M8.3,14 L11.6,21 M15.7,14 L12.4,21" stroke="url(#towerGradAc)" stroke-width="1.1" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.6,7.6 Q7.5,7.8 5,9.3 Q7.6,8.3 9.8,8.5 Z" fill="url(#wingGradAc)"/><path d="M14.4,7.6 Q16.5,7.8 19,9.3 Q16.4,8.3 14.2,8.5 Z" fill="url(#wingGradAc)"/><path d="M9,12.6 Q6.3,12.9 4,14.7 Q6.9,13.4 9.2,13.6 Z" fill="url(#wingGradAc)"/><path d="M15,12.6 Q17.7,12.9 20,14.7 Q17.1,13.4 14.8,13.6 Z" fill="url(#wingGradAc)"/><path d="M5,9.3 L5,10.3 M19,9.3 L19,10.3 M4,14.7 L4,15.7 M20,14.7 L20,15.7" stroke="#9aa8b0" stroke-width="1" stroke-linecap="round"/><ellipse cx="5" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradAc)"/><ellipse cx="19" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradAc)"/><ellipse cx="4" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradAc)"/><ellipse cx="20" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradAc)"/></svg></div>
  ...
  <div class="flow-bottom-wrap">
    ...
    <div class="icons-row bottom">
      <div class="icon-item">
        <div class="icon-circle"><svg class="tower-svg" viewBox="0 0 24 24" width="24" height="24"><defs><linearGradient id="towerGradCa" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#c2d0d8"/><stop offset="1" stop-color="#7c8f9c"/></linearGradient><linearGradient id="wingGradCa" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4a6072"/><stop offset="1" stop-color="#1e2a35"/></linearGradient><linearGradient id="insulatorGradCa" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#9df0fb"/><stop offset="1" stop-color="#1fa8c2"/></linearGradient></defs><rect x="3.5" y="21" width="17" height="1.8" rx="0.9" fill="#12161b"/><rect x="3.5" y="21" width="17" height="0.6" rx="0.3" fill="#333b43" opacity="0.7"/><path d="M12,1.6 L7.2,21 M12,1.6 L16.8,21" stroke="url(#towerGradCa)" stroke-width="1.7" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.3,8.6 L14.7,8.6 M9.3,8.6 L14.7,14 M14.7,8.6 L9.3,14 M8.3,14 L15.7,14 M8.3,14 L11.6,21 M15.7,14 L12.4,21" stroke="url(#towerGradCa)" stroke-width="1.1" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.6,7.6 Q7.5,7.8 5,9.3 Q7.6,8.3 9.8,8.5 Z" fill="url(#wingGradCa)"/><path d="M14.4,7.6 Q16.5,7.8 19,9.3 Q16.4,8.3 14.2,8.5 Z" fill="url(#wingGradCa)"/><path d="M9,12.6 Q6.3,12.9 4,14.7 Q6.9,13.4 9.2,13.6 Z" fill="url(#wingGradCa)"/><path d="M15,12.6 Q17.7,12.9 20,14.7 Q17.1,13.4 14.8,13.6 Z" fill="url(#wingGradCa)"/><path d="M5,9.3 L5,10.3 M19,9.3 L19,10.3 M4,14.7 L4,15.7 M20,14.7 L20,15.7" stroke="#9aa8b0" stroke-width="1" stroke-linecap="round"/><ellipse cx="5" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradCa)"/><ellipse cx="19" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradCa)"/><ellipse cx="4" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradCa)"/><ellipse cx="20" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradCa)"/></svg></div>
  ...
  <script>
    ...
    async function refresh() {
      ...
      try {
        ...
        if (d.source_emoji === '🔌') {
          sourceEmojiEl.innerHTML = '<svg class="tower-svg" viewBox="0 0 24 24" width="24" height="24"><defs><linearGradient id="towerGradSrc" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#c2d0d8"/><stop offset="1" stop-color="#7c8f9c"/></linearGradient><linearGradient id="wingGradSrc" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#4a6072"/><stop offset="1" stop-color="#1e2a35"/></linearGradient><linearGradient id="insulatorGradSrc" x1="0" y1="0" x2="0" y2="1"><stop offset="0" stop-color="#9df0fb"/><stop offset="1" stop-color="#1fa8c2"/></linearGradient></defs><rect x="3.5" y="21" width="17" height="1.8" rx="0.9" fill="#12161b"/><rect x="3.5" y="21" width="17" height="0.6" rx="0.3" fill="#333b43" opacity="0.7"/><path d="M12,1.6 L7.2,21 M12,1.6 L16.8,21" stroke="url(#towerGradSrc)" stroke-width="1.7" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.3,8.6 L14.7,8.6 M9.3,8.6 L14.7,14 M14.7,8.6 L9.3,14 M8.3,14 L15.7,14 M8.3,14 L11.6,21 M15.7,14 L12.4,21" stroke="url(#towerGradSrc)" stroke-width="1.1" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M9.6,7.6 Q7.5,7.8 5,9.3 Q7.6,8.3 9.8,8.5 Z" fill="url(#wingGradSrc)"/><path d="M14.4,7.6 Q16.5,7.8 19,9.3 Q16.4,8.3 14.2,8.5 Z" fill="url(#wingGradSrc)"/><path d="M9,12.6 Q6.3,12.9 4,14.7 Q6.9,13.4 9.2,13.6 Z" fill="url(#wingGradSrc)"/><path d="M15,12.6 Q17.7,12.9 20,14.7 Q17.1,13.4 14.8,13.6 Z" fill="url(#wingGradSrc)"/><path d="M5,9.3 L5,10.3 M19,9.3 L19,10.3 M4,14.7 L4,15.7 M20,14.7 L20,15.7" stroke="#9aa8b0" stroke-width="1" stroke-linecap="round"/><ellipse cx="5" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradSrc)"/><ellipse cx="19" cy="11" rx="1.5" ry="1" fill="url(#insulatorGradSrc)"/><ellipse cx="4" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradSrc)"/><ellipse cx="20" cy="16.4" rx="1.5" ry="1" fill="url(#insulatorGradSrc)"/></svg>';
        } else {
    ...
    // dev.fits: mismo punto que "Gestión de cargas" (ver _compute_device_fits
    // en el backend), pegado acá para no tener que mirar dos secciones
    // separadas para saber "está prendido" y "me lo puedo permitir" a la vez.
    function fitDot(dev) {
        ...
        if self.path == "/api/devices":
            ...
            try:
                ...
                if not (0 <= pct <= 100):
                    ...
                    return
                shared_state.BATTERY_LOW_THRESHOLD = pct
                shared_state._save_persisted_state()
                payload = json.dumps({"threshold_pct": shared_state.BATTERY_LOW_THRESHOLD}).encode("utf-8")
                self._send(200, payload, "application/json")
            except Exception as exc:
                payload = json.dumps({"error": str(exc)}).encode("utf-8")
                self._send(500, payload, "application/json")
